...
const PostCreate = ({route}) => {
    ...
    useEffect(() => {
        const getData = async () => {
                try{ 
                    const apiresponse =  await axios.get("http://192.168.1.4:8080/api/v1/postdesc/count",{
                    headers:{
    ...
    if (!count) {
      ...
      const handlePublish = async() =>{
        try{  
            ...
            apiresponse = await axios.post("http://192.168.1.4:8080/api/v1/postdesc/create",payload,{
              headers:{
                "Content-Type":'application/json'
              }
            })
